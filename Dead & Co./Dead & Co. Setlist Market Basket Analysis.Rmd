...
#looks at all 125 shows' setlists
#inspect(Dead)
...
itemFrequency(Dead)["Let The Good Times Roll"]

#rules for hearing a specific song
rules <- apriori(Dead, parameter = list(supp = (5/length(Dead)), conf = .70, minlen = 2, maxlen = 4),appearance = list(default = "lhs", rhs = "Brokedown Palace"), control = list(verbose = FALSE));inspect(sort(rules,by="confidence"))
...
```{r}
rules <- apriori(Dead, parameter = list(supp = 5/length(Dead), conf = 0.60, minlen=2, maxlen = 2),control=list(verbose=FALSE))
length(rules)
